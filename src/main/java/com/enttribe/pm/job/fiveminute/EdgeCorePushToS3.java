...
public class EdgeCorePushToS3 extends Processor {
    ...
    private static final Logger logger = LoggerFactory.getLogger(EdgeCorePushToS3.class);

    @Override
    public Dataset<Row> executeAndGetResultDataframe(JobContext jobContext) throws Exception {
        logger.info("====EDGECORE PUSH TO S3 STARTED====");
        ...
        logger.info("====EDGECORE PUSH TO S3 COMPLETED====");
        return dataFrame;
    }
    ...
    private Map<String, String> prepareCheckpointOptions(JobContext jobContext) {
        ...
        if (Utils.hasValidValue(this.checkPointLoc)) {
            ...
            String checkpointLocation = SparkRunnerUtils.getCheckpointLocation(topicName,
                    this.checkPointLoc);
            logger.info("EDGECORE TOPICS ==> {} AND CHECKPOINT LOCATION ==> {}", topicName, checkpointLocation);
    ...
    private VoidFunction2<Dataset<Row>, Long> createBatchFunction() {
        return (batchDF, batchId) -> {
            ...
            logger.info("EDGECORE PROCESSING BATCH ID ==> {}", batchId);
            ...
            for (Row row : rows) {
                ...
                logger.info("EDGECORE WRITING ORC TO ==> {}", orcPath);
                ...
                try {
                    ...
                    logger.info("EDGECORE SUCCESSFULLY WROTE ORC TO ==> {}", orcPath);

                } catch (Exception e) {
                    logger.error("EDGECORE FAILED WRITING ORC TO ==> {}", orcPath, e);
                }
            }

            long endTime = System.currentTimeMillis();
            long durationMs = endTime - startTime;
            logger.info("EDGECORE BATCH ID {} PROCESSED IN {} MS", batchId, durationMs);
        };
    }
