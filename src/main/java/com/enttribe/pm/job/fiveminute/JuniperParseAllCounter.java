...
public class JuniperParseAllCounter implements
    ...
    private void parseAllCounter(List<Row> fileContent, String dateTime, String category, String pmemsId,
        ...
        try {
            ...
            if (time != null && !time.isEmpty()) {
                ...
                if (time.length() == 4) {
                    ...
                    if (minutes >= 0 && minutes < 15) {
                        quarterKey = dateKey + String.format("%04d", hours * 100);
                    } else if (minutes >= 15 && minutes < 30) {
                        quarterKey = dateKey + String.format("%04d", hours * 100 + 15);
                    } else if (minutes >= 30 && minutes < 45) {
                        quarterKey = dateKey + String.format("%04d", hours * 100 + 30);
                    } else if (minutes >= 45 && minutes < 60) {
                        quarterKey = dateKey + String.format("%04d", hours * 100 + 45);
                    }
                } else {
                    timeValue = Integer.parseInt(time);
                    if (timeValue >= 0 && timeValue < 15) {
                        quarterKey = dateKey + "0000";
                    } else if (timeValue >= 15 && timeValue < 30) {
                        quarterKey = dateKey + "0015";
                    } else if (timeValue >= 30 && timeValue < 45) {
                        quarterKey = dateKey + "0030";
                    } else if (timeValue >= 45 && timeValue < 60) {
                        quarterKey = dateKey + "0045";
                    }
                }
            }
            String frequency = jobcontext.getParameter("JOB_TYPE");
            ...
            logger.info("🔍 Extracted Pmemsid: {}", extractedPmemsid);
