...
public class PMHourlyJobTrigger {

    public static JobContext jobContext = null;

    private static Logger logger = LoggerFactory.getLogger(PMHourlyJobTrigger.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            dataFrame = TRANSPORT_ALL_VENDOR_HOURLY_KPI_COMPUTATION.callSubFlow(dataFrame, jobContext);
        } catch (Exception e) {
    ...
    private static JobContext setParametersToJobContext(JobContext jobContext) {
        ...
        jobContext.setParameters("NE_TYPES", "'INTERFACE','ROUTER'");
        jobContext.setParameters("NODES", "INTERFACE,ROUTER");
        jobContext.setParameters("EMS_TYPE", "NA");
        jobContext.setParameters("JOB_TYPE", "HOURLY");
        // jobContext.setParameters("RAW_FILE_PATH_SUFFIX", "20251017/0000");
        ...
        // jobContext.setParameters("IS_FILE_FILTER_REQUIRED", "true");
        jobContext.setParameters("KPI_ORC_PATH", "s3a://performance/JOB/ORC/frequency=QUARTERLY/");

        List<String> FOUND_ORC_FILES_PATHS = new ArrayList<>();
        FOUND_ORC_FILES_PATHS.add(
                "s3a://performance/JOB/ORC/frequency=QUARTERLY/domain=TRANSPORT/vendor=JUNIPER/emstype=NA/technology=COMMON/date=251017/time=0000/ptime=*/categoryname=*");
        FOUND_ORC_FILES_PATHS.add(
                "s3a://performance/JOB/ORC/frequency=QUARTERLY/domain=TRANSPORT/vendor=JUNIPER/emstype=NA/technology=COMMON/date=251017/time=0015/ptime=*/categoryname=*");
        FOUND_ORC_FILES_PATHS.add(
                "s3a://performance/JOB/ORC/frequency=QUARTERLY/domain=TRANSPORT/vendor=JUNIPER/emstype=NA/technology=COMMON/date=251017/time=0030/ptime=*/categoryname=*");
        jobContext.setParameters("FOUND_ORC_FILES", String.join(",", FOUND_ORC_FILES_PATHS));
        jobContext.setParameters("CQL_TABLE_NAME", "combinehourlypm");
