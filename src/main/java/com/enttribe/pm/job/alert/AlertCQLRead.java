...
public class AlertCQLRead extends Processor {

    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory.getLogger(AlertCQLRead.class);
    ...
    public AlertCQLRead() {
        super();
        logger.info("AlertCQLRead No Argument Constructor Called!");
    }

    public AlertCQLRead(Dataset<Row> dataframe, Integer id, String processorName) {
        super(id, processorName);
        this.dataFrame = dataframe;
        logger.info("AlertCQLRead Constructor Called with Input DataFrame With ID: {} and Processor Name: {}", id,
                processorName);
    }

    public AlertCQLRead(Integer id, String processorName) {
        super(id, processorName);
        logger.info("AlertCQLRead Constructor Called with ID: {} and Processor Name: {}", id, processorName);
    }
    ...
    public Dataset<Row> executeAndGetResultDataframe(JobContext jobContext) throws Exception {
        ...
        logger.info("[AlertCQLRead] Execution Started!");
        ...
        if (rowList == null || rowList.isEmpty()) {
            logger.info("No Rows Found in DataFrame. Returning Empty DataFrame.");
            return this.dataFrame;
        }

        Row row = rowList.get(0);
        if (row == null) {
            logger.info("First Row is NULL. Returning Empty DataFrame.");
            return this.dataFrame;
        }

        Object inoutConfigurationsObj = row.getAs("INPUT_CONFIGURATIONS");
        ...
        Object extractedParametersObj = row.getAs("EXTRACTED_PARAMETERS");
        ...
        Object kpiCodeNameMapObj = row.getAs("KPI_CODE_NAME_MAP");
        kpiCodeNameMapJson = kpiCodeNameMapObj != null ? kpiCodeNameMapObj.toString() : null;
        if (kpiCodeNameMapJson == null) {
            logger.info("KPI_CODE_NAME_MAP is NULL. Returning Empty DataFrame.");
            return this.dataFrame;
        }

        jobContext.setParameters("KPI_CODE_NAME_MAP", kpiCodeNameMapJson);
        logger.info("KPI_CODE_NAME_MAP Set to Job Context Successfully!");
        ...
        logger.info("Input Config Map: {}", inputConfigMap);
        ...
        logger.info("KPI Code Name Map: {}", kpiCodeNameMap);
        ...
        logger.info("[AlertCQLRead] Execution Completed! Time Taken: {} Minutes | {} Seconds", minutes, seconds);
    ...
    private static Dataset<Row> getCQLDataForSelectedLevel(JobContext jobContext, String aggregationLevel,
        ...
        logger.info("Processing Parameters - Domain: {}, Vendor: {}, Technology: {}, Timestamp: {}", domain, vendor, technology, timestamp);
    ...
    private static Dataset<Row> getCQLDataUsingSpark(String cqlFilter, JobContext jobContext,
        ...
        // Get all Spark configurations and print them
        // scala.collection.immutable.Map<String, String> scalaMap =
        // jobContext.sqlctx().getAllConfs();

        // Map<String, String> javaMap =
        // scala.collection.JavaConverters.mapAsJavaMap(scalaMap);

        // javaMap.forEach((key, value) -> {
        // logger.info("Key: {}, Value: {}", key, value);
        // });

        // String timestampForNode = jobContext.getParameter("TIMESTAMP");

        // DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd
        // HH:mm:ss.SSSSSSxxxx");
        // OffsetDateTime odt = OffsetDateTime.parse(timestampForNode, formatter);

        // 2. Convert to UTC time (optional but safe)
        // OffsetDateTime utcTime = odt.withOffsetSameInstant(ZoneOffset.UTC);

        // 3. Extract date in "yyyyMMdd" format
        // String date = utcTime.format(DateTimeFormatter.ofPattern("yyyyMMdd")); // 👉
        // gives "20250119"

        // 4. Convert to java.sql.Timestamp for Spark
        // java.sql.Timestamp ts = java.sql.Timestamp.from(utcTime.toInstant());
        ...
        while (!success && currentRetry < maxRetries) {
            try {
                ...
                // "pushdown", "true"))
                // .load()
                // .filter(functions.col("domain").equalTo(filterMap.get("domain"))
                // .and(functions.col("vendor").equalTo(filterMap.get("vendor")))
                // .and(functions.col("technology").equalTo(filterMap.get("technology")))
                // .and(functions.col("datalevel").equalTo(filterMap.get("datalevel")))
                // .and(functions.col("date").equalTo(date))
                // .and(functions.col("nodename").equalTo(filterMap.get("nodename")))
                // .and(functions.col("timestamp").equalTo(ts))); // 👈 this

                // Map<String, String> options = new HashMap<>();
                // options.put("keyspace", "pm");
                // options.put("table", "combinequarterlypm");
                // options.put("spark.cassandra.connection.host",
                // "demo-dc1-service.k8ssandra-operator.svc.cluster.local");
                // options.put("spark.cassandra.connection.port", "9042");
                // options.put("spark.cassandra.connection.local_dc", "dc1");
                // options.put("spark.cassandra.auth.username", "cassandra");
                // options.put("spark.cassandra.auth.password", "cassandra");

                // Timestamp ts = Timestamp.valueOf(filterMap.get("timestamp"));
                // String tsString = filterMap.get("timestamp");

                // tsString = tsString.replace(" ", "T");
                // DateTimeFormatter formatter =
                // DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.SSSSSSxxxx");
                // OffsetDateTime odt = OffsetDateTime.parse(tsString, formatter);
                ...
                // .and(functions.col("technology").equalTo(filterMap.get("technology")))
                // .and(functions.col("datalevel").equalTo(filterMap.get("datalevel")))
                // .and(functions.col("date").equalTo(filterMap.get("date")))
                // .and(functions.col("nodename").equalTo(filterMap.get("nodename")))
                // .and(functions.col("timestamp").equalTo(instant)));
