...
public class FinalPMAlertCloseMain {

        public static JobContext jobContext = null;

        private static Logger logger = LoggerFactory.getLogger(FinalPMAlertCloseMain.class);

        public static void main(String[] args) {

                long startTime = System.currentTimeMillis();
                logger.info("🚀 FinalPMAlertCloseMain Execution Started!");
                ...
                try {
                        ...
                        String READ_OPEN_ALERT_RULES_QUERY = "SELECT ALERTID, " +
                                        "UPPER(JSON_UNQUOTE(JSON_EXTRACT(REPLACE(REPLACE(CONFIGURATION,'\"',''),\"'\",'\"'),'$.closureexpression'))) AS EXPRESSION, "
                                        +
                                        "UPPER(JSON_UNQUOTE(JSON_EXTRACT(REPLACE(REPLACE(CONFIGURATION,'\"',''),\"'\",'\"'),'$.closuredescription'))) AS DESCRIPTION, "
                                        +
                                        "NAME AS ALARM_NAME, DOMAIN, VENDOR, TECHNOLOGY, CONFIGURATION " +
                                        "FROM PERFORMANCE_ALERT " +
                                        "WHERE DOMAIN = '$DOMAIN' " +
                                        "AND VENDOR = '$VENDOR' " +
                                        "AND TECHNOLOGY = '$TECHNOLOGY' " +
                                        "AND UPPER(JSON_UNQUOTE(JSON_EXTRACT(REPLACE(REPLACE(CONFIGURATION,'\"',''),\"'\",'\"'),'$.closureFrequency[0]'))) = '$FREQUENCY' "
                                        +
                        ...
                        dataFrame.show();
                        logger.info("++++++++++++[READ CLOSE ALERT RULES]++++++++++++");
                        ...
                        CloseAlertExtractConf closeAlertExtractConf = new CloseAlertExtractConf();
                        closeAlertExtractConf.dataFrame = dataFrame;
                        dataFrame = closeAlertExtractConf.executeAndGetResultDataframe(jobContext);
                        ...
                        for (int i = currentCount; i < finalCount; i++) {
                                ...
                                CloseAlertCQLRead closeAlertCQLRead = new CloseAlertCQLRead();
                                closeAlertCQLRead.dataFrame = dataFrame;
                                dataFrame = closeAlertCQLRead.executeAndGetResultDataframe(jobContext);

                                ProcessCloseCQLResult processCloseCQLResult = new ProcessCloseCQLResult();
                                processCloseCQLResult.dataFrame = dataFrame;
                                processCloseCQLResult.executeAndGetResultDataframe(jobContext);
                                currentCount++;
                        ...
                        logger.error("Exception Occurred in FinalPMAlertCloseMain: {}", e.getMessage());
                        e.printStackTrace();
                }

                long endTime = System.currentTimeMillis();
                long executionTime = endTime - startTime;
                logger.info("🚀 FinalPMAlertCloseMain Execution Completed in {} Minutes {} Seconds",
                                executionTime / 60000,
        ...
        private static SparkConf getSparkConf() {
                ...
                return new SparkConf()
                                .setAppName("FinalPMAlertCloseMain")
                                .setMaster("local[*]")
