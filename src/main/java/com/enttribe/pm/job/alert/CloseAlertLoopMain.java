...
public class CloseAlertLoopMain {

    public static JobContext jobContext = null;

    private static Logger logger = LoggerFactory.getLogger(CloseAlertLoopMain.class);

    public static void main(String[] args) {

        long startTime = System.currentTimeMillis();
        logger.info("🚀 CloseAlertLoopMain Execution Started!");
        ...
        try {
            ...
            AlertUDFUpdated alertUDF = new AlertUDFUpdated();
            ...
            PMAlertExtraction pmAlertExtraction = new PMAlertExtraction();
            pmAlertExtraction.dataFrame = dataFrame;
            dataFrame = pmAlertExtraction.executeAndGetResultDataframe(jobContext);
            ...
            for (int i = currentCount; i < finalCount; i++) {
                ...
                AlertCQLReadLoop alertCQLReadLoop = new AlertCQLReadLoop();
                alertCQLReadLoop.dataFrame = dataFrame;
                dataFrame = alertCQLReadLoop.executeAndGetResultDataframe(jobContext);

                ProcessCQLResultLoop processCQLResultLoop = new ProcessCQLResultLoop();
                processCQLResultLoop.dataFrame = dataFrame;
                processCQLResultLoop.executeAndGetResultDataframe(jobContext);
                currentCount++;
            ...
            logger.error("Exception Occurred in CloseAlertLoopMain: {}", e.getMessage());
            e.printStackTrace();
        }

        long endTime = System.currentTimeMillis();
        long executionTime = endTime - startTime;
        logger.info("🚀 CloseAlertLoopMain Execution Completed in {} Minutes {} Seconds", executionTime / 60000,
