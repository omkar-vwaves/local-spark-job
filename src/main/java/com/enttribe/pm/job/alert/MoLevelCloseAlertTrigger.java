...
public class MoLevelCloseAlertTrigger {

    public static JobContext jobContext = null;

    private static Logger logger = LoggerFactory.getLogger(MoLevelCloseAlertTrigger.class);

    public static void main(String[] args) {

        long startTime = System.currentTimeMillis();
        logger.info("🚀 MoLevelCloseAlertTrigger Execution Started!");
        ...
        try {
            ...
            String READ_PM_ALERT_CONFIGURATION_QUERY = "SELECT JSON_UNQUOTE(JSON_EXTRACT(CAST(CONFIGURATION AS JSON), '$.closureexpression')) AS EXPRESSION, JSON_UNQUOTE(JSON_EXTRACT(CAST(CONFIGURATION AS JSON), '$.closuredescription')) AS DESCRIPTION, ALERTID FROM PERFORMANCE_ALERT WHERE DOMAIN = '"
                    + domain
                    + "' AND VENDOR = '" + vendor + "' AND TECHNOLOGY = '" + technology
                    + "' AND UPPER(JSON_UNQUOTE(JSON_EXTRACT(CAST(CONFIGURATION AS JSON), '$.closureFrequency[0]'))) = '"
            ...
            dataFrame = new JDBCReadCustom(dataFrame, 2, "READ ACTIVE ALARM LIBRARY",
            ...
            String JOIN_ACTIVE_ALARM_LIBRARY_WITH_PM_ALERT_CONFIGURATION_QUERY = "SELECT al.ALARM_IDENTIFIER AS ALARM_EXTERNAL_ID, al.ALARM_ID AS ALARM_CODE, pfa.EXPRESSION, pfa.DESCRIPTION FROM PERFORMANCE_ALERT pfa JOIN ALARM_LIBRARY al ON pfa.ALERTID = al.ALARM_IDENTIFIER";
            ...
            dataFrame = new ExecuteSparkSQLD1Custom(dataFrame, 3,
                    "JOIN ACTIVE ALARM LIBRARY WITH PM ALERT CONFIGURATION",
                    JOIN_ACTIVE_ALARM_LIBRARY_WITH_PM_ALERT_CONFIGURATION_QUERY, "JOIN_RESULT", null)
                    .executeAndGetResultDataframe(jobContext);
        ...
        logger.info("🚀 MoLevelCloseAlertTrigger Execution Completed in {} Minutes {} Seconds", executionTime / 60000,
    ...
    private static SparkConf getSparkConf() {
        ...
        return new SparkConf()
                .setAppName("MoLevelCloseAlertTrigger")
                .setMaster("local[*]")
