...
public class AdvaParserForDelta implements UDF4<String, String, byte[], Boolean, List<Row>>, AbstractUDF {
    ...
    private void getCategoryVsDeltaCounterMap() {
        if (categoryVsDelta == null) {
            synchronized (SYNCHRONIZER) {
                if (categoryVsDelta == null) {
                    logger.info("[getCategoryVsDeltaCounterMap] - Starting initialization of categoryVsDelta map");
                    ...
                    try {
                        ...
                        logger.info("[getCategoryVsDeltaCounterMap] - categoryVsDelta map initialized successfully with size: {}", 
                                   categoryVsDelta.size());
                    } catch (Exception e) {
                        logger.error("[getCategoryVsDeltaCounterMap] - Error during initialization of categoryVsDelta map: {}", 
                                    e.getMessage(), e);
                    }
                }
            }
    ...
    public List<Row> call(String zipFilePath, String fileName, byte[] csvContent, Boolean isDelta) throws Exception {
        logger.info("AdvaParser: Processing File: {} With {} Bytes", fileName, csvContent.length);
        logger.info("AdvaParser: Received Delta Mode: {}", isDelta);
        ...
        // Initialize category vs delta map
        getCategoryVsDeltaCounterMap();

        int index = 1;
        for (CSVRecord record : records) {
            try {
                logger.debug("AdvaParser: Processing Record: {} With Record: {}", index, record);
                ...
                logger.debug("AdvaParser: Counter Name: {}, Entity Name: {}, Interface Name: {}, Period: {}, Value: {}", 
                           counterName, entityName, interfaceName, period, value);
                ...
                if (interfaceName != null && interfaceName.contains("OM-")) {
                ...
                counterName = counterName != null ? counterName.replace("(", "").replace(")", "").trim().toUpperCase() : null;
                entityName = entityName != null ? entityName.replace("(", "").replace(")", "").trim().toUpperCase() : null;
                interfaceName = interfaceName != null ? interfaceName.replace("(", "").replace(")", "").trim().toUpperCase() : null;
                ...
                String baseKey = FIXED_CATEGORY + "##" + pmemsid;

                logger.debug("AdvaParser: Generated Pmemsid: {}, Base Key: {}", pmemsid, baseKey);

                // Create counter value maps
                Map<String, String> counterValueMap = new HashMap<>();
                Map<String, String> deltaCounterValueMap = new HashMap<>();

                // Add counter value to appropriate maps
                if (counterName != null && value != null) {
                    counterValueMap.put(counterName, value);
                    
                    // Check if this counter is a delta counter
                    List<String> deltaKeys = categoryVsDelta != null ? categoryVsDelta.get(FIXED_CATEGORY) : null;
                    if (deltaKeys != null && deltaKeys.contains(counterName)) {
                        deltaCounterValueMap.put(counterName, value);
                    }
                }

                // Always add interface description
                if (interfaceName != null) {
                    counterValueMap.put("INTERFACE_DESC", interfaceName);
                    if (!deltaCounterValueMap.isEmpty()) {
                        deltaCounterValueMap.put("INTERFACE_DESC", interfaceName);
                    }
                }

                // Create rows following Cisco pattern
                // First create delta row if delta counters exist
                if (!deltaCounterValueMap.isEmpty()) {
                    fileContent.add(RowFactory.create(processingTime, baseKey, deltaCounterValueMap, true, dateTime));
                }
                
                // Then create the main counter row
                fileContent.add(RowFactory.create(processingTime, baseKey, counterValueMap, false, dateTime));
                
                index++;

            } catch (Exception e) {
                logger.error("AdvaParser: Exception While Processing CSV Line: {}", record, e);
            }
        }
        
        logger.info("AdvaParser: Total rows created: {}", fileContent.size());
        return fileContent;
    }
    ...
    private String getProcessedTimeFromZipFilePath(String zipFilePath) {
        try {
            ...
            logger.error("AdvaParser: Error Extracting Processed Time From Zip File Path: {}", zipFilePath);
        }
        return "0000";
    }
    ...
    private String getTimeForQuarterly(String timeKey, Integer mins) throws ParseException {
        try {
            ...
            logger.error("AdvaParser: Error Extracting Time For Quarterly: {}, Mins: {} With Error: {}", 
                        timeKey, mins, e.getMessage());
        }
        return DEFAULT_PROCESSING_TIME;
    }
